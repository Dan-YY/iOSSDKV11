	...
	public func addButtonAndTitle() {
		...
		if let rootView = UIApplication.shared.keyWindow {
			if #available(iOS 11.0, *) {
				...
				safeAreaHeight = rootView.bounds.height - topInset - bottomInset
			} else {
				safeAreaHeight = rootView.bounds.height
			}
		}

		backButton = UIButton(frame: CGRect(x: 20, y: safeAreaHeight * 0.065, width: 40, height: 40))

		if backButtonImage != nil {
			print("backbutton image not nil")
			backButton.setImage(backButtonImage, for: .normal)
		} else {
			print("backbutton image is nil")
		...
		if let cameraTitle = cameraTitle {
			...
			let textFieldSize = CGSize(width: screenSize.width * 0.75, height: 40)
			let textView = UILabel(frame: CGRect(origin: CGPoint.zero, size: textFieldSize))
			textView.textAlignment = .center
			//textView.numberOfLines = 1
			textView.textColor = .white
			textView.text = cameraTitle
			textView.font = UIFont.preferredFont(forTextStyle: .title3)
			textView.center = backButton.center
			textView.center = CGPoint(x: screenSize.width * 0.5, y: backButton.center.y)
			self.view.addSubview(textView)
		}
	}

	@objc func backTapped(_ sender: Any){
		self.cameraCaptureDelegate?.setCapturedImage(image: Image(), barcodeString: "cancelled")
